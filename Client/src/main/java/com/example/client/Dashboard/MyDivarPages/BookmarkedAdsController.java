...
public class BookmarkedAdsController {
    ...
    public void initialize() {
        int size = (int) Math.ceil((double) GetInfo.getSizeOfArray("bookmarkPost") / 8);
        if (size != 0) {
    ...
    private Node CreatePage(int pageIndex) {
        ...
        ArrayList<String> list = GetInfo.getMarkedPost(8, pageIndex);
        for (String str : list) {
            userPosts.add(new JSONObject(str));
        }
        VBox vBox = new VBox();
        NewPage newPage = new NewPage(userPosts, vBox, "BookmarkedAds");
